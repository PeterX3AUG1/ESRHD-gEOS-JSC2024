...
//    u = Arr(npxx + 2 * gpn, VEC_DIM);
    ...
    err.open("tolEntr_1DSmooth80_EOS_3_EC6_quad.txt", ios::ate);
}
        ...
        for (rk_ite = 0; rk_ite < RK_ORDER; rk_ite++) {
            ...
            for (int i = 0; i < npxx + 2 * gpn; i++) {
                ...
                if (rk_ite == 0 && i >= gpn && i < npxx + gpn) {
                    ...
                    REAL SEntr = 1.5Q * logq(upri[i][PRE_IND]) - 2.5Q * logq(upri[i][RHO_IND]) + 1.5Q * logq(h - theta);
                    S += -upri[i][RHO_IND]/ sqrtq(1-powq(upri[i][VV_XXIND],2))*SEntr;
                }
