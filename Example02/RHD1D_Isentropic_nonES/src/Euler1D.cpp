...
//        dt = pow(dt, 6.0 / RK_ORDER);
        ...
        for (rk_ite = 0; rk_ite < RK_ORDER; rk_ite++) {
            ...
            //flux
            if (rk_ite == 0) {
                ComputeFlux(EC_order, ES_order, t);
            }
            if (rk_ite == 1) {
                ComputeFlux(EC_order, ES_order, t + dt);
            }
            if (rk_ite == 2) {
                ComputeFlux(EC_order, ES_order, t + 0.5 * dt);
            }
//            ComputeFlux(EC_order, ES_order);
        ...
        t = t + dt;
        std::cout << std::setprecision(16) << scientific << "t = " << t << " " << u[npxx / 2 + gpn][RHO_IND] << std::endl;
...
void Euler1D::ComputeFlux(int &EC_order, int &ES_order, double t_RK) {

    //Flux high order Entropy Conservation and LF
    ComputFluxESAndLF(EC_order, ES_order, t_RK);
...
void Euler1D::ComputFluxESAndLF(int &EC_order, int &ES_order, double t_RK) {
...
#if DISSIPATION
            fluxES[i][k] -= 1.2*(sin(50*t_RK)) * Dissip[k];  // non-EC/non-ES scheme
//            fluxES[i][k] -= Dissip[k];
#endif
        }//
