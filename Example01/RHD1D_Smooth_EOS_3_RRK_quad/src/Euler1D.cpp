        ...
        for (rk_ite = 0; rk_ite < RK_ORDER; rk_ite++) {
            ...
            for (int i = gpn; i < npxx + gpn; i++) {
                for (int k = 0; k < VEC_DIM; k++) {
                    ...
                    if (rk_ite == 1) {
                        finte_2[i][k] = finte;
                        urk[rk_to_update][i][k] = urk[0][i][k] + 0.25Q * lambda * (finte_1[i][k] + finte);
                        if (rk_to_update != rk_ite + 1) {
                            cout << "error: rk_to_update != rk_ite + 1" << std::endl;
                            exit(1);
                        }
...
//        t = t + dt;
        t = t + gam_n * dt;
        printf("t = %.16Qf, dt = %.16Qf, %.16Qf, gam_n = %.32Qf\n", t, dt, u[npxx / 2 + gpn][RHO_IND], gam_n);
//        std::cout << "t=" << t << "," << "dt=" << dt << " " << u[npxx / 2 + gpn][RHO_IND] << std::setprecision(16) << scientific << ", gam_n = " << gam_n << std::endl;
...
REAL Euler1D::Find_relaxation_gamma(REAL ddt, REAL** conU_RRK, REAL** finte_1_RRK, REAL** finte_2_RRK, REAL** finte_3_RRK,
    ...
    while (f0 > TOL && k_iter < N) {
        ...
        for (int i = gpn; i < npxx + gpn; i++) {
            ...
            for (int k = 0; k < VEC_DIM; k++) {
                dn[i][k] = ddt * (finte_1_RRK[i][k] + finte_2_RRK[i][k] + 4 * finte_3_RRK[i][k]) / 6.0Q / dxx;
        ...
        for (int i = gpn; i < npxx + gpn; i++) {
            REAL theta = priU_RRK[i][PRE_IND] / priU_RRK[i][RHO_IND];
            REAL ht = 2.5Q * theta + sqrtq(1.0Q + 2.25Q * theta * theta);
            eta += -conU_RRK[i][RHO_IND] * (1.5Q*logq(priU_RRK[i][PRE_IND]) - 2.5Q * logq(priU_RRK[i][RHO_IND]) + 1.5Q * logq(ht - theta));
            REAL theta_gam = priU_plus_dnx[i][PRE_IND] / priU_plus_dnx[i][RHO_IND];
            REAL ht_gam = 2.5Q * theta_gam + sqrtq(1.0Q + 2.25Q * theta_gam * theta_gam);
            eta_gam += -conU_plus_dnx[i][RHO_IND] * (1.5Q*logq(priU_plus_dnx[i][PRE_IND]) - 2.5Q * logq(priU_plus_dnx[i][RHO_IND]) + 1.5Q * logq(ht_gam - theta_gam));
        }
        eta = eta * dxx;
        eta_gam = eta_gam * dxx;
