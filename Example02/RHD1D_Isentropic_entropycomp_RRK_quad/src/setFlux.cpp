...
void setEntropyConserFlux(REAL &BigG, REAL *priVL, REAL *priVR, REAL *flux) {
    ...
    REAL e_hat = 1 + 1 / (BigG - 1) / beta_log;
    REAL dh_hat = (p + d_log * e_hat) / (powq(w_Lor,2) - powq(z_2,2));
    flux[FF_MM_XXIND] = dh_hat * powq(z_2,2) + p;
...
void setLFFlux(REAL &BigG, REAL *conVL, REAL *conVR, REAL *flux) {
    ...
    REAL e_L = pre_L / (BigG - 1) / rho_L, e_R = pre_R / (BigG - 1) / rho_R;
    REAL h_L = 1 + e_L + pre_L / rho_L, h_R = 1 + e_R + pre_R / rho_R;

    REAL gas_csL = sqrtq(BigG * priVL[PRE_IND] / priVL[RHO_IND] / h_L);
    REAL gas_csR = sqrtq(BigG * priVR[PRE_IND] / priVR[RHO_IND] / h_R);
