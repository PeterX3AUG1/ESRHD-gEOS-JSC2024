...
void setLFFluxXX(REAL &BigG, REAL *conVL, REAL *conVR, REAL *flux) {
    ...
    REAL e_L = theta_L - 1.0 + sqrt(1.0 + 4.*theta_L * theta_L), e_R = theta_R - 1.0 + sqrt(1.0 + 4.*theta_R * theta_R);
    REAL h_L = 1 + e_L + pre_L / rho_L, h_R = 1 + e_R + pre_R / rho_R;

    REAL gas_csL = sqrt(2.*theta_L / (4.*theta_L + sqrt(1.0 + 4.*theta_L * theta_L)));
    REAL gas_csR = sqrt(2.*theta_R / (4.*theta_R + sqrt(1.0 + 4.*theta_R * theta_R)));
