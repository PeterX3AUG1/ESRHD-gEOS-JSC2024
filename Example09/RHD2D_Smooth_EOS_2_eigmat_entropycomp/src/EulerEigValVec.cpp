...
REAL SetEigVecScalingXX(const REAL *priVL, const REAL *priVR,
    ...
    const REAL betasqinv4p1 = (sqrt(1.0 + 4./betaL/betaL) + sqrt(1.0 + 4./betaR/betaR)) / 2.0;
    const REAL betainv2 = (2./betaL + 2./betaR) / 2.0;
    const REAL p = d / beta;
    ...
    const REAL p_log = d_log / beta_log;
     const REAL betasqinv4p1_pbetainv2 = ave_log(2./betaL + sqrt(1.0 + 4./betaL/betaL), 2./betaR + sqrt(1.0 + 4./betaR/betaR));
    REAL e_hat = 1./beta_log + betasqinv4p1 - betainv2 / beta * (beta * betainv2 / betasqinv4p1 - 2.*(1.0 + betainv2 / betasqinv4p1) / betasqinv4p1_pbetainv2);
...
//    REAL Bx = d_log*w_Lor + p_log*w_Lor/(coeff-h_log)-tau[2]*w_Lor*w_Lor*v2*v2;
    REAL Bx = d_log*w_Lor*(cv/cp-theta_log*v2*v2/h_log/(1-v1*v1)); // 2023/4/13
