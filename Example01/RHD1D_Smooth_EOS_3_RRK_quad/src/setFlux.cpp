...
void setEntropyConserFlux(REAL &BigG, REAL *priVL, REAL *priVR, REAL *flux) {
    ...
    const REAL betasqinv4p1 = (sqrtq(1.0Q + 2.25Q/betaL/betaL) + sqrtq(1.0Q + 2.25Q/betaR/betaR)) / 2.0Q;
    const REAL betainv2 = (1.5Q/betaL + 1.5Q/betaR) / 2.0Q;
    const REAL p = d / beta;

    const REAL d_log = ave_log(dL, dR);
    const REAL beta_log = ave_log(betaL, betaR);
    const REAL betasqinv4p1_pbetainv2 = ave_log(1.5Q/betaL + sqrtq(1.0Q + 2.25Q/betaL/betaL), 1.5Q/betaR + sqrtq(1.0Q + 2.25Q/betaR/betaR));
    const REAL p_log = d_log / beta_log;


    const REAL z_1 = d; // 算术平均
    const REAL z_2 = (w_Lor_L * v1L + w_Lor_R * v1R) / 2.0Q; // 算术平均
    const REAL z_3 = beta; // 算术平均

    // 熵守恒通量：
    flux[FF_RHO_IND] = d_log * z_2;
    REAL e_hat = 1.5Q/beta_log + betasqinv4p1 - betainv2 / beta * (beta * betainv2 / betasqinv4p1 - 1.5Q*(1.0Q + betainv2 / betasqinv4p1) / betasqinv4p1_pbetainv2);
    REAL dh_hat = (p + d_log * e_hat) / (w_Lor * w_Lor - z_2 * z_2);
    flux[FF_MM_XXIND] = dh_hat * z_2 * z_2 + p;
...
void setLFFlux(REAL &BigG, REAL *conVL, REAL *conVR, REAL *flux) {
    ...
    REAL e_L = 1.5Q * theta_L - 1.0Q + sqrtq(1.0Q + 2.25Q*theta_L * theta_L), e_R = 1.5Q * theta_R - 1.0Q + sqrtq(1.0Q + 2.25Q*theta_R * theta_R);
    REAL h_L = 1 + e_L + pre_L / rho_L, h_R = 1 + e_R + pre_R / rho_R;

    REAL gas_csL = sqrtq((9.0Q*theta_L * theta_L + 10.0Q*theta_L * sqrtq(1.0Q + 2.25Q*theta_L * theta_L))
                         / 6.0Q / (6.0Q*theta_L * theta_L + 4.0Q*theta_L * sqrtq(1.0Q + 2.25Q*theta_L * theta_L) + 1.0Q));
    REAL gas_csR = sqrtq((9.0Q*theta_R * theta_R + 10.0Q*theta_R * sqrtq(1.0Q + 2.25Q*theta_R * theta_R))
                         / 6.0Q / (6.0Q*theta_R * theta_R + 4.0Q*theta_R * sqrtq(1.0Q + 2.25Q*theta_R * theta_R) + 1.0Q));

    REAL eigL = (fabsq(priVL[VV_XXIND]) + gas_csL) / (1 + fabsq(priVL[VV_XXIND]) * gas_csL);
    REAL eigR = (fabsq(priVR[VV_XXIND]) + gas_csR) / (1 + fabsq(priVR[VV_XXIND]) * gas_csR);
    REAL max_eig = fmaxq(eigL, eigR);
