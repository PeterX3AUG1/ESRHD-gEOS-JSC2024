...
void Euler1D::Evolve(int &EC_order, int &ES_order) {
    ...
    while (t_ite < stop_ite && small_ite < 1e8) {
        ...
        max_eig_xx = Global_MaxEig(1);
        max_eig_yy = Global_MaxEig(0);
...
//        dt = pow(dt, 5.0 / RK_ORDER);
        ...
        t = t + dt;
        std::cout << "t=" << t << "," << "dt=" << dt << " " << u[npxx / 2 + gpn][npyy / 2 + gpn][RHO_IND] << std::endl;
//        exit(0);
...
void Euler1D::ComputFluxESAndLFXX(int &EC_order, int &ES_order) {
    ...
    for (int j = gpn; j < npyy + gpn; j++) {
        ...
        REAL Dissip[VEC_DIM] = {0.0};
            ...
            dissipation(1,ES_order, BigG, priU_left, priU_right,
