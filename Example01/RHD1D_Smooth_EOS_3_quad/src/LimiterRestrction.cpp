...
REAL WENOGL41(REAL v5, REAL v4, REAL v3, REAL v2, REAL v1) {
    ...
    tmp1 = v1 - 2 * v2 + v3;
    tmp2 = v1 - 4 * v2 + 3 * v3;
    s1 = 13.0Q / 12 * tmp1 * tmp1 + 0.25Q * tmp2 * tmp2 + eps;
    s1 = 0.1Q / (s1 * s1);

    tmp1 = v2 - 2 * v3 + v4;
    tmp2 = v2 - v4;
    s2 = 13.0Q / 12.0 * tmp1 * tmp1 + 0.25Q * tmp2 * tmp2 + eps;
    s2 = 0.6Q / (s2 * s2);

    tmp1 = v3 - 2 * v4 + v5;
    tmp2 = 3 * v3 - 4 * v4 + v5;
    s3 = 13.0Q / 12.0Q * tmp1 * tmp1 + 0.25Q * tmp2 * tmp2 + eps;
    s3 = 0.3Q / (s3 * s3);
    ...
    return (w1 * (v1 / 3.0Q - 7.0Q * v2 / 6.0Q + 11.0Q * v3 / 6.0Q) + w2 * (-v2 / 6.0Q + 5.0Q * v3 / 6.0Q + v4 / 3.0Q) +
            w3 * (v3 / 3.0Q + 5.0Q * v4 / 6.0Q - v5 / 6.0Q));
}
...
REAL WENOGL42(REAL v5, REAL v4, REAL v3, REAL v2, REAL v1) {
    ...
    tmp1 = v1 - 2 * v2 + v3;
    tmp2 = v1 - 4 * v2 + 3 * v3;
    s1 = 13.0Q / 12.0Q * tmp1 * tmp1 + 0.25Q * tmp2 * tmp2 + eps;

    s1 = (91.0Q + 9.0Q * sq5) / (440.0Q * s1 * s1);

    tmp1 = v2 - 2 * v3 + v4;
    tmp2 = v2 - v4;
    s2 = 13.0Q / 12.0Q * tmp1 * tmp1 + 0.25Q * tmp2 * tmp2 + eps;
    ...
    value = w1 * ((-1.0Q / 60 + 0.05Q * sq5) * v1 + (1.0Q / 30 - sq5 * 0.2Q) * v2 + (59.0Q / 60 + 0.15Q * sq5) * v3) +
            w2 * ((-1.0Q / 60 - 0.05Q * sq5) * v2 + 31.0Q / 30 * v3 + (-1.0Q / 60 + 0.05Q * sq5) * v4) +
            w3 * ((59.0Q / 60 - 0.15Q * sq5) * v3 + (1.0Q / 30 + 0.2Q * sq5) * v4 + (-1.0Q / 60 - 0.05Q * sq5) * v5);
    return value;
}
...
REAL WENOPW44(REAL v1, REAL v2, REAL v3, REAL v4, REAL v5) {
    ...
    tmp1 = (4.0Q * v1 * v1 - 19.0Q * v1 * v2 + 25.0Q * v2 * v2 + 11.0Q * v1 * v3 - 31.0Q * v2 * v3 + 10.0Q * v3 * v3) / 3.0Q;
    s1 = fabsq(tmp1) + eps;
    s1 = (1.0Q / 16.0Q) / (s1 * s1);

    tmp1 = (4.0Q * v2 * v2 - 13.0Q * v2 * v3 + 13.0Q * v3 * v3 + 5.0Q * v2 * v4 - 13.0Q * v3 * v4 + 4.0Q * v4 * v4) / 3.0Q;
    s2 = fabsq(tmp1) + eps;
    s2 = (5.0Q / 8.0Q) / (s2 * s2);

    tmp1 = (10.0Q * v3 * v3 - 31.0Q * v3 * v4 + 25.0Q * v4 * v4 + 11.0Q * v3 * v5 - 19.0Q * v4 * v5 + 4.0Q * v5 * v5) / 3.0Q;
    s3 = fabsq(tmp1) + eps;
    s3 = (5.0Q / 16.0Q) / (s3 * s3);
    ...
    return (w1 * (0.375Q * v1 - 1.25Q * v2 + 1.875Q * v3) + w2 * (-0.125Q * v2 + 0.75Q * v3 + 0.375Q * v4) +
            w3 * (0.375Q * v3 + 0.75Q * v4 - 0.125Q * v5));
}
...
//	{ -0.3125,  0.0625, -0.0625,  0.3125,  2.1875} };
    ...
    for (l = 2; l <= k; l++) {
        if (fabsq(dd[l - 1][k - curR - 2]) <= fabsq(dd[l - 1][k - curR - 1]))
            curR++;
    }
