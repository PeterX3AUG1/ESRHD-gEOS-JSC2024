...
void Euler1D::Init() {
    ...
    xxbl = 0.0, xxbr = 325.0;  // shock_bubble
    yybl = 0.0, yybr = 90.0;
    ...
    for (int i = 0; i < npxx + gpn * 2; i++) {
        for (int j = 0; j < npyy + gpn * 2; j++) {
            ...
            if ((xx-215.0)*(xx-215.0)+(yy-45.0)*(yy-45.0)<=625.0)
            {
                uu = 0.0;
                vv = 0.0;
                den = 3.1538;
            ...
            {
                uu = -0.200661045980881;
                vv = 0.0;
                den = 1.941272902134272;
                pre = 0.15;
            }
...
//                u[npxx + gpn - 1 + i][j][k] = u[npxx + gpn - 1][j][k];
    ...
    /** inflow B.C. **/
    REAL d = 1.941272902134272, vx = -0.200661045980881, p = 0.15;
    REAL w = 1./ sqrt(1-vx*vx), tem = p/d, h = 2.*(6.*tem*tem+4.*tem+1.0)/(3.*tem+2.0);
    for (int j = gpn; j <= npyy + gpn - 1; j++) {
        for (int i = 1; i <= gpn; i++) {
//            for (int k = 0; k < VEC_DIM; k++) {
//                u[gpn - i][j][k] = u[gpn][j][k];
                u[npxx + gpn - 1 + i][j][RHO_IND] = d*w;
                u[npxx + gpn - 1 + i][j][VV_XXIND] = d*h*w*w*vx;
                u[npxx + gpn - 1 + i][j][VV_YYIND] = 0.0;
                u[npxx + gpn - 1 + i][j][PRE_IND] = d*h*w*w-p;
//            }
        }
    }
    ...
    for (int i = 0; i < npxx + 2 * gpn; i++) {
        for (int j = 1; j <= gpn; j++) {
            for (int k = 0; k < VEC_DIM; k++) {
                if (k == VEC_DIM - 2) {
                    u[i][gpn - j][k] = -u[i][gpn+j-1][k];
                    u[i][npyy + gpn - 1 + j][k] = -u[i][npyy + gpn - j][k];
                }
                else {
                    u[i][gpn - j][k] = u[i][gpn+j-1][k];
                    u[i][npyy + gpn - 1 + j][k] = u[i][npyy + gpn - j][k];
...
void Euler1D::OutputOrder() {
    ...
    ofstream os("2DRHD_shockbb_2_EOS_numsol_caxis90.dat", ios::ate);

    REAL conU[VEC_DIM], priU[VEC_DIM];
    REAL xx, yy, rho_min = 1000.0, pre_min = 1000.0, rho_max = 0.0;
    ...
    for (int i = gpn; i < npxx + gpn; i++) {
        for (int j = gpn; j < npyy + gpn; j++) {
            ...
            rho_min = fmin(rho_min, priU[RHO_IND]); rho_max = fmax(rho_max, priU[RHO_IND]);
