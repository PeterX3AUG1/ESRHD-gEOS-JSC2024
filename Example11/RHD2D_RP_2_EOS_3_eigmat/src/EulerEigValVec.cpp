...
REAL SetEigVecScalingXX(const REAL *priVL, const REAL *priVR,
    ...
    const REAL betasqp4 = (sqrt(betaL * betaL + 9./ 4.0) + sqrt(betaR * betaR + 9./ 4.0)) / 2.0;
    const REAL p = d / beta;
    ...
    const REAL betasqp4p2_log = ave_log(1.5 + sqrt(betaL * betaL + 9./ 4.0), 1.5 + sqrt(betaR * betaR + 9./ 4.0));
    const REAL h_log = beta / betasqp4 * (1.0 - 1.5 /betasqp4p2_log) + 4./beta_log;
    ...
    i = 1;
    REAL coeff = 1./ sqrt(1.0 + 2.25 *theta_log * theta_log); // 1+e+\rho*e_r
    MatR[0][i] = 1.0 / w_Lor;
    MatR[1][i] = v1*coeff;
    MatR[2][i] = v2*coeff;
    MatR[3][i] = coeff;
