...
REAL Hybrid_Root_p(REAL D, REAL mx, REAL my, REAL E, REAL BigG) {
    ...
    REAL TOL = 1e-15;
    REAL a = 0, b = 2.* E / 3.0; // 9/27: EOS #2 二分法上界改变值
    REAL x = b / 2.0;
    REAL k = 0; // 当前迭代步数
    REAL N = log(b*1e15) / log(2) + 1; // 二分法预估迭代步数，9/25, XLF, test pure bisection method
    REAL f0 = 1.0 + TOL;
    REAL f, temp;
    while (f0 > TOL && k < N) {
...
//    std::cout << "xx_vel = " << xx_vel << std::endl;
...
//    std::cout << "w_Lor = " << w_Lor << std::endl;
    REAL theta = pre / rho; // 9/25: EOS #1
    REAL e = theta - 1.0 + sqrt(1.0 + 4.*theta * theta); // 9/27: EOS #2
