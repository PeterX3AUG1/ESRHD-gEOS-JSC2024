...
//    const REAL w_Lor = (w_Lor_L + w_Lor_R) / 2.0;
    const REAL w_Lor = 1./sqrt(1-v1*v1-v2*v2); // 2023/4/13
    // 密度取算术平均: {{\rho}}_{i+1/2}
    const REAL d = (dL + dR) / 2.0;
    // beta取算术平均: {{\beta}}_{i+1/2}
    const REAL beta = (betaL + betaR) / 2.0;
    const REAL betasqinv4p1 = (sqrt(1.0 + 9./betaL/betaL/4.0) + sqrt(1.0 + 9./betaR/betaR/4.0)) / 2.0;
    const REAL betainv2 = (1.5/betaL + 1.5/betaR) / 2.0;
    const REAL p = d / beta;
    ...
    const REAL betasqinv4p1_pbetainv2 = ave_log(1.5/betaL + sqrt(1.0 + 9./betaL/betaL/4.0), 1.5/betaR + sqrt(1.0 + 9./betaR/betaR/4.0));
    REAL e_hat = 1.5/beta_log + betasqinv4p1 - betainv2 / beta * (beta * betainv2 / betasqinv4p1 - 1.5*(1.0 + betainv2 / betasqinv4p1) / betasqinv4p1_pbetainv2);

    const REAL h_log = e_hat + 1./beta_log; // 2023/4/12

    const REAL theta_log = 1./ beta_log;

    const REAL cp = 2.5 + 2.25 *theta_log / sqrt(1.0 + 2.25 *theta_log * theta_log);
    const REAL cv = 1.5 + 2.25 *theta_log / sqrt(1.0 + 2.25 *theta_log * theta_log);
    ...
    REAL tau[VEC_DIM];
    tau[1] = d_log*pow(w_Lor,3)/cp; // 2023/4/13
    tau[2] = p_log/h_log/(1.0-v1*v1)/w_Lor;
//    REAL Bx = d_log*w_Lor + p_log*w_Lor/(coeff-h_log)-tau[2]*w_Lor*w_Lor*v2*v2;
    REAL Bx = d_log*w_Lor*(cv/cp-theta_log*v2*v2/h_log/(1.0-v1*v1)); // 2023/4/13
    REAL Cx = p_log*v1*sqrt(1-v1*v1-v2*v2+cssq)*(w_Lor*w_Lor*(1-v1*v1)-cssq*(v1*v1+w_Lor*w_Lor*v2*v2))/h_log/gas_cs/w_Lor/w_Lor/(1-v1*v1)/(1-v1*v1)/(1-vsq*cssq);
    tau[0] = (Bx-Cx) / 2.0;
    tau[3] = (Bx+Cx) / 2.0;
...
REAL SetEigVecScalingYY(const REAL *priVL, const REAL *priVR,
    ...
    tau[2] = d_log*pow(w_Lor,3)/cp; // 2023/4/13
//    REAL By = d_log*w_Lor+p_log*w_Lor/(coeff-h_log)-tau[1]*w_Lor*w_Lor*v1*v1;
    REAL By = d_log*w_Lor*(cv/cp-theta_log*v1*v1/h_log/(1.0-v2*v2)); // 2023/4/13
    REAL Cy = p_log*v2*sqrt(1-v2*v2-v1*v1*cssq)*(w_Lor*w_Lor*(1-v2*v2)-cssq*(v2*v2+w_Lor*w_Lor*v1*v1))/h_log/gas_cs/w_Lor/w_Lor/(1-v2*v2)/(1-v2*v2)/(1-vsq*cssq);
    tau[0] = (By-Cy) / 2.0;
    tau[3] = (By+Cy) / 2.0;
...
REAL MaxEigValue(REAL &BigG, int flag, const REAL *priU) {
    ...
    REAL theta = pre / rho;
    REAL gas_cs = sqrt((9.*theta * theta + 10.*theta * sqrt(1.0 + 2.25 * theta * theta))
                       / 6.0 / (6.*theta * theta + 4.*theta * sqrt(1.0 + 2.25 * theta * theta) + 1.0));
    REAL cssq = gas_cs * gas_cs;
