...
//    u = Arr(npxx + 2 * gpn, VEC_DIM);
    ...
    err.open("tolEntr_1DRP2_100_RRK_ES5_SO.txt", ios::ate);
}
        ...
        for (rk_ite = 0; rk_ite < RK_ORDER; rk_ite++) {
            ...
            for (int i = 0; i < npxx + 2 * gpn; i++) {
                ...
                if (rk_ite == 0 && i >= gpn && i < npxx + gpn) {
                    REAL theta = upri[i][PRE_IND] / upri[i][RHO_IND];
                    REAL h = 2.5 * theta + sqrt(1.0 + 9.*theta * theta / 4.0);
                    REAL SEntr = 1.5 * log(upri[i][PRE_IND]) - 2.5 * log(upri[i][RHO_IND]) + 1.5 * log(h - theta);
                    S += -upri[i][RHO_IND]/ sqrt(1-pow(upri[i][VV_XXIND],2))*SEntr;
//                    cout << "S[ " << i << "] = " << S << endl;
                }
...
REAL Euler1D::Find_relaxation_gamma(REAL ddt, REAL** conU_RRK, REAL** finte_1_RRK, REAL** finte_2_RRK, REAL** finte_3_RRK,
    ...
    while (f0 > TOL && k_iter < N) {
        ...
        for (int i = gpn; i < npxx + gpn; i++) {
            ...
            REAL ht = 2.5 * theta + sqrt(1.0 + 2.25 * theta * theta);
            eta += -conU_RRK[i][RHO_IND] * (1.5*log(priU_RRK[i][PRE_IND]) - 2.5 * log(priU_RRK[i][RHO_IND]) + 1.5 * log(ht - theta));
            REAL theta_gam = priU_plus_dnx[i][PRE_IND] / priU_plus_dnx[i][RHO_IND];
            REAL ht_gam = 2.5 * theta_gam + sqrt(1.0 + 2.25 * theta_gam * theta_gam);
            eta_gam += -conU_plus_dnx[i][RHO_IND] * (1.5*log(priU_plus_dnx[i][PRE_IND]) - 2.5 * log(priU_plus_dnx[i][RHO_IND]) + 1.5 * log(ht_gam - theta_gam));
        }
        eta = eta * dxx;
        eta_gam = eta_gam * dxx;
