...
//    u = Arr(npxx + 2 * gpn, VEC_DIM);
    ...
    err.open("tolEntr_1Ddensity100_ES5_T376.txt", ios::ate);
}
        ...
        for (rk_ite = 0; rk_ite < RK_ORDER; rk_ite++) {
            ...
            for (int i = 0; i < npxx + 2 * gpn; i++) {
                ...
                if (rk_ite == 0 && i >= gpn && i < npxx + gpn) {
                    S += -upri[i][RHO_IND]/ sqrt(1-pow(upri[i][VV_XXIND],2))*(1.5*log(3.*upri[i][PRE_IND]+2.*upri[i][RHO_IND]) - 3.*upri[i][RHO_IND]/(3.*upri[i][PRE_IND]+2.*upri[i][RHO_IND]) + 1.5*log(upri[i][PRE_IND]) - 4.*log(upri[i][RHO_IND]));
                }
