...
Euler1D::Euler1D(int nx, int ny) : npxx(nx), npyy(ny), gpn(4) {
    ...
    urk = Arr(4, npxx + 2 * gpn, npyy + 2 * gpn, VEC_DIM);
...
//    u = Arr(npxx + 2 * gpn, VEC_DIM);
    ...
    err.open("tolEntr_2DSmooth80_EOS_EC6.txt", ios::ate);
}
            ...
            for (int i = 0; i < npxx + 2 * gpn; i++) {
                for (int j = 0; j < npyy + 2 * gpn; j++) {
                    ...
                    if (rk_ite == 0 && i >= gpn && i < npxx + gpn && j >= gpn && j < npyy + gpn) {
                        REAL SEntr = 1.5*log(3.*upri[i][j][PRE_IND]+2.*upri[i][j][RHO_IND]) - 3.*upri[i][j][RHO_IND]/(3.*upri[i][j][PRE_IND]+2.*upri[i][j][RHO_IND]) + 1.5*log(upri[i][j][PRE_IND]) - 4.*log(upri[i][j][RHO_IND]);
