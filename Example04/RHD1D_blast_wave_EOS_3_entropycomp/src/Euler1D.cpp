...
Euler1D::Euler1D(int nx) : npxx(nx), gpn(4) {
    ...
    err.open("tolEntr_1Dblast4000_ES5_T43.txt", ios::ate);
}
...
void Euler1D::Evolve(int &EC_order, int &ES_order) {
    ...
    REAL max_eig;
    REAL S;  // discrete entropy at each time step, 23/1/14
...
//        dt = pow(dt, 5.0 / RK_ORDER);
...
#if RELAX_FLAG == 1
                if (rk_ite == 0) {
                    computeEntrFromPri(BigG, upri[i], Entr1[i]); // w(u^n)
                }
                if (rk_ite == 1) {
                    computeEntrFromPri(BigG, upri[i], Entr2[i]); // w(u^(1))
                }
                if (rk_ite == 2) {
                    computeEntrFromPri(BigG, upri[i], Entr2[i]); // w(u^(2))
                }
#endif
            }
        ...
        std::cout << "t=" << t << " " << u[npxx / 2 + gpn][RHO_IND] << ", tolEntr = " << std::setprecision(16) << scientific << S << std::endl;
